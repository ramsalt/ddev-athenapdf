[![tests](https://github.com/drud/ddev-athenapdf/actions/workflows/tests.yml/badge.svg)](https://github.com/drud/ddev-athenapdf/actions/workflows/tests.yml) ![project is maintained](https://img.shields.io/maintenance/yes/2022.svg)

## What is ddev-athenapdf?

This repository is a template for providing [DDEV](https://ddev.readthedocs.io) addons and services.

In ddev v1.19+ addons can be installed from the command line using the `ddev get` command, for example, `ddev get drud/ddev-athenapdf` or `ddev get drud/ddev-drupal9-solr`.
...
* The fundamental contents of the add-on service or other component. For example, in this template there is a [docker-compose.athenapdf.yaml](docker-compose.athenapdf.yaml) file.
...
3. Globally replace "athenapdf" with the name of your add-on.
4. Add the files that need to be added to a ddev project to the repository. For example, you might remove `docker-composeathenapdf.yaml` with the `docker-compose.*.yaml` for your recipe.
